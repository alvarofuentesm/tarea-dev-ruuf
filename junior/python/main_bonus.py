...
def empty_positions(positions: list, roof_height_A: int, roof_width_A: int,
                    roof_height_B: int, roof_width_B: int, 
                    gap_width: int, gap_height: int ):
    
    # representar las posiciones en el roof
        
    for y in range(roof_height_B + roof_height_A - gap_height): # todos vacios al inicio
        positions.append([])
        for x in range(roof_width_A + roof_width_B - gap_width ):
            if (x < (roof_width_A - gap_width)  and y < (roof_height_B - gap_height) ):
                positions[y].append(-1)
            elif (x >= roof_width_A  and y >= roof_height_B) :
                positions[y].append(-1)
            else:
...
def calculate_panels(panel_width: int, panel_height: int,  
                    roof_width_A: int, roof_height_A: int,
                    roof_width_B: int, roof_height_B: int,
                    gap_width: int, gap_height: int) -> int:
    ...
    max_panels = ( roof_height_A * roof_width_A + roof_height_B * roof_width_B - gap_width*gap_height )//(panel_width * panel_height)
    print("max_panels: ", max_panels)
    ...
    best_solution = []
    empty_positions(best_solution, roof_height_A, roof_width_A, roof_height_B, roof_width_B, gap_width, gap_height)
    best_space_left = ( roof_height_A * roof_width_A + roof_height_B * roof_width_B - gap_width*gap_height )
    best_total_panels = 0

    for permutation in   permutations[:len(permutations)//2]: # por simetria del escenario puedo revisar solamente la mitad   
        #print("PERM")
        space_left = ( roof_width_A * roof_height_A + roof_height_B * roof_height_B - gap_width*gap_height ) # funcion de costo
        total_panels = 0
        
        positions = []
        empty_positions(positions, roof_height_A, roof_width_A, roof_height_B, roof_width_B, gap_width, gap_height)

        #show_solution(positions)
        
        total_panels, space_left = add_to_roof_permutation(panel_width, panel_height, 
                                                            roof_width_A + roof_width_B - gap_width,
                                                            roof_height_B + roof_height_A - gap_height,
...
def run_tests() -> None:
    with open('test_cases_bonus.json', 'r') as f:
        data = json.load(f)
        test_cases: List[Dict[str, int]] = [
            {
                "panel_w": test["panelW"],
                "panel_h": test["panelH"],
                "roof_w_A": test["roofWA"],
                "roof_h_A": test["roofHA"],
                "roof_w_B": test["roofWB"],
                "roof_h_B": test["roofHB"],
                "gap_w": test["gapW"],
                "gap_h": test["gapH"],
    ...
    for i, test in enumerate(test_cases, 1):
        result = calculate_panels(
            ...
            test["roof_w_A"], test["roof_h_A"],
            test["roof_w_B"], test["roof_h_B"],
            test["gap_w"], test["gap_h"]
        )
        passed = result == test["expected"]
        
        print(f"Test {i}:")
        print(f"  Panels: {test['panel_w']}x{test['panel_h']}")
        print(f"  Roof A: {test['roof_w_A']}x{test['roof_h_A']}")
        print(f"  Roof B: {test['roof_w_B']}x{test['roof_h_B']}")
        print(f"  gap: {test['gap_w']}x{test['gap_h']}")
